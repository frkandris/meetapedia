...
**settings.yaml schedule flags**: four are left — `worker_enabled`, `enrich_enabled`, `enrich_batch_limit`, and `report_enabled`/`report_cron` (the daily email, and the only cron in the system). Startup no longer runs anything: the worker starts on boot and picks the right work immediately, so an interrupted run needs no recovery — its finished pairs are already cached and its unfinished ones are simply pending again. That is what stops a deploy from costing a collection run, and it replaced `_startup_plan()`, deleted 2026-09-20 with the windows it was written to respect.
